...
export function HotkeyPicker() {
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent) => {
    ...
    const keyInfo = getShortcutCandidate(e);
    if (!keyInfo) {
      if (!isModifierOnlyKey(e.key)) {
        setError(SHORTCUT_SUPPORT_MESSAGE);
      }
      return;
    }

    const validationError = validateShortcut(keyInfo);
    if (validationError) {
  ...
  const saveHotkey = useCallback(async () => {
    ...
    if (!isTauri()) {
      setError('Shortcut updates are only available in the desktop app');
      return;
    }
